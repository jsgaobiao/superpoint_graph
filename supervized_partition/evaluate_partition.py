...
parser.add_argument('--odir', default='./results_part/', help='Directory to store results')
...
if args.dataset == 'skitti':
    i_fold = 0
    for i in args.cvfold.split(' '):
        fold = int(i)
        if args.dataset == 'skitti':
            base_name = root + '{:0>2d}'.format(fold) 
    ...
    for i_fold in range(len(args.cvfold)):
        ...
        # elif args.dataset == 'skitti':
            # base_name = root + '{0>2:d}'.format(fold) 
